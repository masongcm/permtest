...
Saving  
	__savemat__  (_path_): save command output in matrix form to _path_
...
__permtest__ performs permutation-based inference of treatment effects 
...
__treat__ specifies the variable containing the treatment indicator. It must be
binary coded with values of 0 and 1.

__np__ specifies the desired number of permutations of the outcome vector.

__blockvars__ (_varlist_) can be used to supply one or more strata used in the
randomisation protocol. Permutations will then be performed _within_ 
blocks defined by the values (or combination of values, if more than one
stratum is supplied) in __blockvars__. The strata variables should be
categorical, and shouldn't partition the sample in sets that are too small.

__naive__ overrides the __blockvars__ option, by instructing the permutation
to disregard strata and just permute outcomes across the whole sample.

__lcvars__ (_varlist_) can be used to supply a list of variables to linearly 
condition on during inference. The conditioning is performed using the 
methodology from [Freedman and Lane (1983)](https://www.jstor.org/stable/1391660).
Typical usage includes the case of baseline covariate imbalance.

__ipwcovars1__ (_varlist_) can be used to supply a list of variables to perform 
...
study period.
Denote by W_j an indicator for outcome Y_j being observed. The __ipwcovars1__
option computes a set of weights from a logit regression of W_j on _varlist_, 
and uses them to adjust the final treatment effect estimates. 

Note that:

> The logit weights are computed _for each permutation_: as such this option 
significantly increases the time intensity of the command.

> Supplying only __ipwcovars1__ uses the same set of adjustment variables for
each outcome in _depvars_. If you want to specify different sets of adjustment
variables for each outcome, use __ipwcovars2-ipwcovars5__ (currently limited to
5 different outcomes).

__reverse__ prompts all the treatment effect estimates to be reversed in sign.
This is because __permtest__ conducts inference only in the form of one-sided
pvalues, and as such the sign of the outcome variables in _depvars_ matters.

> If the hypotheses to be tested for the variables in _depvars_ have different
signs, the variables will have to be reversed manually before supplying them
to __permtest__.

__effsize__ reports treatment effects on the console as effect sizes, i.e.
standardised using the SD of the control group. This does not affect the results
stored in __r()__, which will include both normal treatment effects and effect
sizes.

__verbose__ provides the option to print more information to the console about
the execution of __permtest__.

__savemat__  (_path_) instructs __permtest__ to save the outputs in __r()__ to
a file on disk, in csv format.
...
[mason.gcm@gmail.com](mailto:mason.gcm@gmail.com)  
